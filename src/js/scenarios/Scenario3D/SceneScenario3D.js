...
class Wall extends THREE.Mesh {
  constructor(color) {
    ...
    /** body */
    this.body = Bodies.rectangle(0, 0, 1, 1, { isStatic: false })
  }
  ...
  update() {
    // Align the Three.js object with the Matter.js body
    this.position.x = this.body.position.x
    this.position.y = -this.body.position.y // Matter.js may have y-axis inverted
    this.rotation.z = -this.body.angle
  }
}
...
export default class SceneScenario3D extends Scene3D {
  ...
  addBubble(x, y) {
    ...
    Composite.add(this.engine.world, newBubble.body)
  }
  ...
  update() {
    ...
    // Update bubbles and rectangles
    this.bubbles.forEach((bubble) => bubble.update())
    this.walls.forEach((wall) => wall.update()) // Make sure walls have an update method

    // Perform rendering
    this.renderer.render(this, this.camera)
  }

  addRectangle(x, y, color = "green", width = 50, height = 20) {
    // Make sure the width and height are correct for your needs, you may use the bubble's radius
    const rectangle = new Wall(color, false) // Ensure this is dynamic, not static

    rectangle.setSize(width, height) // Set this to bubble's diameter if you want it the same size
    rectangle.setPosition(x, this.height + height / 2) // Start just above the top of the scene
    this.add(rectangle)
    Composite.add(this.engine.world, rectangle.body) // Add to physics world
    this.walls.push(rectangle) // Track the rectangle
  }
  resize() {
    super.resize()
